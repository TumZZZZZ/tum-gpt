...
class ChatController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // If no title provided, use "TumGPT" initially
        if (! $title) {
            $title = 'TumGPT';
        }
        ...
        return response()->stream(function () use ($request, $chat) {
            ...
            if (app()->environment('testing') || ! config('openai.api_key')) {
                ...
                try {
                    ...
                    @ob_flush();
                    @flush();
                    Log::error('OpenAI stream error', ['error' => $e]);
                }
            }
            ...
            if ($chat && $fullResponse) {
                ...
                // Generate title if this is a new chat with "TumGPT" title
                \Log::info('Checking if should generate title', ['chat_title' => $chat->title]);
                if ($chat->title === 'TumGPT') {
    ...
    public function titleStream(Chat $chat)
    {
        ...
        return response()->eventStream(function () use ($chat) {
            // If title is already set and not "TumGPT", send it immediately
            ...
            while (time() - $startTime < $timeout) {
                ...
                // If title has changed from "TumGPT", send it
                if ($chat->title !== 'TumGPT') {
                    yield new StreamedEvent(
                        event: 'title-update',
                        data: json_encode(['title' => $chat->title])
