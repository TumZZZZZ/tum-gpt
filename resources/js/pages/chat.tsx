...
function ChatWithStream({ chat, auth, flash }: { chat: ChatType | undefined; auth: PageProps['auth']; flash: PageProps['flash'] }) {
    ...
    const [currentTitle, setCurrentTitle] = useState<string>(chat?.title || 'TumGPT');
    ...
    useEffect(() => {
        if (!isStreaming && inputRef.current) {
            ...
            // Trigger title generation if this is an authenticated user with "TumGPT" chat and we have a response
            if (auth.user && chat && currentTitle === 'TumGPT' && data && data.trim()) {
            ...
            {shouldGenerateTitle && auth.user && chat && (
                <TitleGenerator
                    ...
                    onTitleUpdate={(newTitle, isStreaming = false) => {
                        ...
                        document.title = `${newTitle} - TumGPT`;
                    }}
                    onComplete={() => {
